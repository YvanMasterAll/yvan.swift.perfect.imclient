...
open class DefaultRefreshHeader: UIView, RefreshableHeader {
    ...
    override init(frame: CGRect) {
        ...
        if let image = PullToRefreshKitBundle.image("arrow_down") {
    ...
    open func didBeginHideAnimation(_ result:RefreshResult) {
        ...
        case .success:
            ...
            if let image = PullToRefreshKitBundle.image("success") {
                imageView.image = image
            }
        case .failure:
            textLabel.text = textDic[.refreshFailure]
            if let image = PullToRefreshKitBundle.image("failure") {
                imageView.image = image
            }
        case .none:
            textLabel.text = textDic[.pullToRefresh]
